...
export const actions: Actions = {
    ...
    removePoints: async ({params, request}) => {
        const formData = await request.formData()

        const currentPoints = parseInt(formData.get('points') as string)
        const pointsToRemove = parseInt(formData.get('pointsToRemove') as string)
        const studentId = params.studentId
        if(Number.isNaN(pointsToRemove)) {
            return fail(400,  { error: "Please enter a number."})
        }
        try {
            let pointsAdded = await db.update(studentProfileTable).set({points: currentPoints - pointsToRemove}).where(eq(studentProfileTable.studentId, studentId)).returning({points: studentProfileTable.points})

            if(pointsAdded[0].points == pointsToRemove) {
